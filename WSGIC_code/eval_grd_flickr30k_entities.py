    ...
    def grd_eval(self, mode='all'):
        ...
        for lst_idx, anns in tqdm(enumerate(ref)):
            ...
            for num_sent, ann in enumerate(anns['captions']):
                ...
                idx_in_sent = {} #{'woman': [2], 'hair': [7], 'pants': [10], 'shirt': [15], 'jacket': [22]}
                ...
                for pred_idx, class_name in enumerate(pred[img][0]['clss']):
                    if class_name in idx_in_sent:
                        ...
                        overlaps_array.append(img)
                        overlaps_array.append(class_name)
                        overlaps_array.append(overlap.tolist())
        ...
        #with open(r'/mnt/data/flickr30k/result_img/grounding_overlaps.json', 'w') as f: #data.tolist()
        #    json.dump(overlaps_array, f, indent=2)
        if mode == 'all':
            with open(r'/mnt/data/flickr30k/result_img/grounding_overlaps.json', 'w') as f:  # data.tolist()
                json.dump(overlaps_array, f, indent=2)
            with open(r'/mnt/data/flickr30k/result_img/grounding_prec.json', 'w') as f:  # data.tolist()
