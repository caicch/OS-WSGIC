...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.fc = nn.Linear(self.rnn_size * 2, self.rnn_size * 4)
    ...
    def init_attn(self, bsz):
        weight = next(self.parameters())
        return (weight.new_zeros(self.num_layers, bsz, 579))
    ...
    def _forward(self, fc_feats, att_feats, seq, VIT_feats, att_masks=None):
        batch_size = fc_feats.size(0)
        state = self.init_hidden(batch_size) #1, 50, 512
        pre_attn = self.init_attn(batch_size)

        outputs = fc_feats.new_zeros(batch_size, seq.size(1) - 1, self.vocab_size+1)
        if self.opt.att_supervise:
            att_weights = fc_feats.new_zeros(batch_size, self.seq_length, 8, 1, 579)

        # Prepare the features
        p_fc_feats, p_att_feats, pp_att_feats, mean_vit, p_att_masks, re_VIT_feats, verb_out, verb_feats = self._prepare_feature(fc_feats, att_feats, att_masks, VIT_feats)
        ...
        for i in range(seq.size(1) - 1):
            ...
            if self.opt.att_supervise:
                output, state, pre_attn, att_weight = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, VIT_feats, re_VIT_feats, mean_vit, pre_attn, p_att_masks, state)
                #print(att_weight.size()) 145, 512
            else:
                output, state, pre_attn = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, VIT_feats, re_VIT_feats, mean_vit, pre_attn, p_att_masks, state)
            outputs[:, i] = output #outputs 50, 17, 7001, - output = 50, 7001
            if self.opt.att_supervise:
                att_weights[:,i,:, :, :]=att_weight
                #print(att_weight.size())

        if self.opt.att_supervise:
            return (outputs, verb_out, att_weights)
        else:
            return (outputs, verb_out)#, cls_out #50, 17, 7001

    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, VIT_feats, re_VIT_feats, mean_vit, pre_attn, att_masks, state):
        # 'it' contains a word index
        xt = self.embed(it)

        if self.opt.att_supervise:
            output, state, pre_attn, att_weight = self.core(xt, fc_feats, att_feats, p_att_feats, VIT_feats, re_VIT_feats, mean_vit, pre_attn, state, att_masks)
        else:
            output, state, pre_attn = self.core(xt, fc_feats, att_feats, p_att_feats, VIT_feats, re_VIT_feats, mean_vit, pre_attn, state, att_masks)
        logprobs = F.log_softmax(self.logit(output), dim=1) #50, 7001

        if self.opt.att_supervise:
            return logprobs, state, pre_attn, att_weight
        else:
            return logprobs, state, pre_attn

    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        beam_size = opt.get('beam_size', 10)
        batch_size = fc_feats.size(0)

        p_fc_feats, p_att_feats, pp_att_feats, mean_vit, p_att_masks = self._prepare_feature(fc_feats, att_feats, att_masks)
    ...
    def _sample(self, fc_feats, att_feats, VIT_feats, att_masks=None, opt={}):
        #print("image_vit:", VIT_feats.size())
        ...
        for t in range(self.seq_length + 1):
            ...
            if self.opt.att_supervise:
                logprobs, state, pre_attn, att_weight = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, VIT_feats, re_VIT_feats, mean_vit, pre_attn, p_att_masks, state)
                #print("output_att_weight:", att_weight.size())
            else:
                logprobs, state, pre_attn = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, VIT_feats, re_VIT_feats, mean_vit, pre_attn, p_att_masks, state)
            ...
            if unfinished.sum() == 0:
                break
        if self.opt.att_supervise:
            return seq, seqLogprobs, att_weights, verb_feats
        else:
...
class TopDownCore(nn.Module):
    ...
    def forward(self, xt, fc_feats, att_feats, p_att_feats, state, att_masks=None):
        ...
        if self.opt.att_supervise:
            ...
            #print(att.size()) #145, 512
            #print('---------------')
            #print(att_weight.size()) #145, 36
        else:
...
class Attention(nn.Module):
    ...
    def forward(self, h, att_feats, p_att_feats, att_masks=None):
        #print('p_att_feats', p_att_feats.size()) #145, 36, 512
        # The p_att_feats here is already projected
        att_size = att_feats.numel() // att_feats.size(0) // att_feats.size(-1)
        att = p_att_feats.view(-1, att_size, self.att_hid_size)
        #print('att', att.size()) #145, 36, 512
        
        att_h = self.h2att(h)                        # batch * att_hid_size
        att_h = att_h.unsqueeze(1).expand_as(att)            # batch * att_size * att_hid_size 145, 36, 512
        dot = att + att_h                                   # batch * att_size * att_hid_size
        dot = torch.tanh(dot)                                # batch * att_size * att_hid_size
        #print("dot:", dot.size())
        dot = dot.view(-1, self.att_hid_size)               # (batch * att_size) * att_hid_size
        #print("dot:", dot.size()) #5220, 512
        dot = self.alpha_net(dot)                           # (batch * att_size) * 1
        #print("dot:", dot.size()) #5220, 1
        dot = dot.view(-1, att_size)                        # batch * att_size
        #print("dot:", dot.size()) #145, 36
        #======================================================================================
        # Sparse attention.
        # pdb.set_trace()
