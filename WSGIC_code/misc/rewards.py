...
def get_self_critical_reward(model, fc_feats, att_feats, images, att_masks, data_gts, gen_result, opt):
    ...
    with torch.no_grad():
        if opt['att_supervise']:
            greedy_res, _, _, _ = model(fc_feats, att_feats, images, att_masks=att_masks, mode='sample')
        else:
            greedy_res, _ = model(fc_feats, att_feats, images, att_masks=att_masks, mode='sample')
    model.train()
